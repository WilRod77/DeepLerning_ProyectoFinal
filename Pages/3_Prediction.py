...
def load_model(modelName): 
    with modelopen(f'Models/Data/model{modelName}.pkl', 'rb') as gb: 
        model=pickle.load(gb) 
    return model 

model_text=st.selectbox("Select a model", options=["Bayesian","Random Forest","ExtraTrees",
...
try:
    ...
    st.sidebar.success(f"{model_text} Model Loaded") 
except:
    st.subheader(f"{model_text} Model Not Loaded") 

st.subheader("Machine Learning Model Selected") 

st.subheader(model_text,"is a algorithm used as a Machine Learning Model") 
st.write("Define of the algorithm implemented for predict the flow patterns") 

features=['Velocidad superficial del líquido (Vsl)', 'Velocidad superficial del gas (Vsg)', 
          'Viscosidad del líquido (VisL)', 'Viscosidad del gas (VisG)', 'Densidad del líquido (DenL)',
            'Densidad del gas (DenG)', 'Tensión superficial (ST)', 'Ángulo de inclinación tubería (Ang)', 
            'Diámetro de la tubería (ID)'] 
st.write("to continue, select the features") 
